...
`MARZBAN_NODE_BINARY_URL` 应指向给目标 Linux 架构构建的 Rust `MarzbanX-node` binary。`SING_BOX_VERSION` 默认固定为稳定版 `1.13.14`，`SING_BOX_DOWNLOAD_URL_TEMPLATE` 默认使用官方 GitHub release archive，所以 HY2 和 AnyTLS 的 Add Node 流程不需要额外手动配置 sing-box 安装源。`SING_BOX_INSTALL_SCRIPT_URL` 保留为 fallback 或内部镜像入口。
...
| `SING_BOX_INSTALL_SCRIPT_URL` | sing-box node 的安装脚本 URL，默认 `https://sing-box.app/install.sh`。 |
| `SING_BOX_VERSION` | 一键安装脚本要求的 sing-box 固定版本，默认 `1.13.14`。 |
| `SING_BOX_DOWNLOAD_URL_TEMPLATE` | install script fallback 之前优先使用的 release archive URL 模板，支持 `{version}` 和 `{arch}`。 |
